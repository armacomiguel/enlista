...
const Counter = ({desc, quiantity, type}:CounterProps) => {
 ...
 const iconName = useMemo(() => {
  ...
  return (
    <View className={`mt-3 ${bgColor} w-[38%] p-3 rounded-md overflow-hidden`}>
      <View className='w-full flex-col items-start'>
        <View className='h-[22vh] overflow-hidden'>
          <Text  style={{fontSize: 65, color: "white" }} className='mb-1 font-bold'>{quiantity}</Text>
          <Text  style={{ fontFamily: 'LilitaOne_400Regular', fontSize: 18, }} className={textColor}>{desc}</Text>
         </View>
